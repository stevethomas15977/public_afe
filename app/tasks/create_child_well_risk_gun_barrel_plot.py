...
class CreateChildWellRiskGunBarrelPlot(Task):

    def execute(self):
        task = TASKS.CREATE_CHILD_WELL_RISK_GUN_BARREL_PLOT.value
        ...
        try:
            ...
            title = f"{self.context.project.capitalize()} child well risk "
            ...
            for i, offset in enumerate(offsets):
                ...
                y_offset = -85
                ax.annotate(str(well.first_production_date), (offset.gun_barrel_x, offset.subsurface_depth + y_offset), color='black', fontsize=5, ha='center', va='center', alpha=0.70, zorder=6)

                # Annotate the perforation interval
                y_offset = 85
                ax.annotate(str(well.perf_interval), (offset.gun_barrel_x, offset.subsurface_depth + y_offset), color='black', fontsize=5, ha='center', va='center', alpha=0.70, zorder=6)
            ...
            for target_well in target_wells:
                ...
                for xyz_distance in list[XYZDistance](xyz_distances):
                   ...
                   # Calculate distances for end points
                    ...
                    text += f"- {api_to_index[reference_analysis.api]}-{api_to_index[target_analysis.api]} ({x_distance_end}, {y_distance_end}, {hyp_distance_end}, "
            ...
            # Save the plot
            output_file = os.path.join(self.context.project_path, f"{self.context.project}-{plot_name}-gun-barrel-plot-{self.context.version}.png")
